...
#
# Removes the application, the monitoring stack, and the Swarm Agent. Does NOT
# touch k3s system pods or python-shell. Safe to run when some pieces are
# already gone (uses --ignore-not-found).
#
# The monitoring stack is deployed into swarm-system alongside the Swarm Agent,
# so removing that namespace removes both. The by-name deletes below are only
# for the cluster-scoped pieces, and for a stack left in "default" by an
# earlier version.
#
# Usage:
...
echo "=== 1/3 Removing the application ($APP) ==="
...
echo "=== 2/3 Removing the monitoring stack ==="
if kubectl get pod python-shell -n default >/dev/null 2>&1; then
  echo "  python-shell found -> clean undeploy_monitoring"
  kubectl exec python-shell -n default -- python3 -c "from swchmonclient import undeploy_monitoring; import sys; sys.exit(undeploy_monitoring(namespace='swarm-system'))" || true
fi
# cluster-scoped pieces are not removed with the namespace
...
# a stack an earlier version left in the default namespace
...
echo "=== 3/3 Removing the Swarm Agent (and the monitoring stack in its namespace) ==="
